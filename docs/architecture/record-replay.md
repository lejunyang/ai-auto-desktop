...
设计隐含假设「编辑后的录制随时可编译」。实测第一次跑通编辑核心即证伪：

编辑 locator **必须**清空 `disambiguation`（否则改过的 locator 会继承它
从未挣得的证明，而这正是 `verify_locators` 存在的理由）；但编译器**拒绝**
unresolved locator。若用编译器校验每次编辑，「改 locator → 再校验」这个
最普通的修复流程将**永远无法保存**。

两条规则都对，错的是把它们混为一谈。二者回答不同问题：

- **saveable**：这是不是一份操作者可以继续工作的良构录制；
- **compilable**：它能不能变成 workflow 去回放。

实现上，保存只做结构校验（重复 id、assertion 指向不存在的步骤——后者正是
§6 实测编译器**不管**的那一类）；可编译性由 `compile_preview()` 独立报告，
并常驻 UI 的「编译结果」面板。操作者在编辑途中就能看到「为什么还不能跑」，
而不是被拦在保存这一步。

### 14.2 重排：警告而非拒绝

§6 已实测三类重排错误。补测的第三类给出了明确结论：把 logic 步骤重排到
它的前置交互之前，编译器**照单全收**（产出 `['guarded', 'open_dialog__snapshot', ...]`）。
静态检查看不见这种破坏。

因此重排只**警告**不拒绝：拒绝会挡住有意的重构，沉默则会掩盖真实破坏。
两个警告码：`ORDER_DIVERGED`（顺序偏离原始录制，并列出原始顺序）、
`ASSERTION_BEFORE_TARGET`（assertion 排到了它的 target 之前）。
后者已双向验证——target 仍在 assertion 之前时**不**触发。

### 14.3 字段用 allow-list

`EDITABLE_FIELDS` = description / enabled / locator / window / text / logic。
其余一律拒绝，包括 `disambiguation`——若可写，UI 就能伪造一次它从未做过的校验。
用 allow-list 而非 deny-list：录制格式将来新增字段时，不会因为没人记得去保护它
而默认可编辑。

删除步骤同样不提供，只提供**禁用**：禁用无损、不产生悬空引用。

### 14.4 威胁模型与一个自引入漏洞

初版有一个未认证的 `/bootstrap` 端点返回 token。**实测**：一个独立子进程
fetch 该端点拿到 token，随即成功读取会话状态。端口虽由 OS 分配，但本地扫描
65535 个端口只需数秒。该端点把 token 降格成了装饰品。

已删除该路由，改为在服务页面时把 token 注入页面。页面本就只经回环提供，
不扩大暴露面，但消除了那个唯一「一请求即奉上」的端点。

**残余暴露（如实记录）**：本地进程仍可读取页面并解析出 token。回环不做进程隔离，
任何放在进程内的 token 都挡不住这一点。token 防的是走失的浏览器标签页或
另一 origin 的页面，**不防**已以本用户身份运行的任意代码。

### 14.5 自动保存：因为退出路径不可依赖

**实测（Windows）**：`terminate()` 与 `CTRL_BREAK_EVENT` 均**不**触发
`KeyboardInterrupt`，也**不**执行 `finally`。初版在退出时才写出编辑结果，
实测直接导致一次完整会话的编辑**全部丢失**。

现改为每次编辑提交后立即写盘，undo 同样触发写盘（否则文件会保留操作者
已经撤销的编辑）。写盘经临时文件后 `os.replace`，因为被杀死于写入中途
会留下一个截断的录制——那比丢掉最后一次编辑更糟。

写盘失败**不**回滚编辑：屏幕上已经生效的编辑若被静默撤销，界面与文件
将对「录制是什么」产生分歧。

编辑结果写到 `--output` 或 stdout，**不覆盖输入文件**：录制无法廉价重拍，
一次编辑会话不应能摧毁它的起点。

### 14.6 早期拒绝必须先排空请求体

`POST` 到未知端点返回的是 `ConnectionAbortedError` 而非 404。原因：401 与 404
在**读取请求体之前**就应答并关闭，未读的请求体仍在途中，Windows 因此重置连接。

这两种早期拒绝恰好是最需要向操作者解释的两种——过期的 token 与错误的端点——
却都会在浏览器里表现为一个不透明的网络错误。且它取决于请求体是否先于关闭抵达，
属于容易被当作「偶发抖动」而忽略的故障。现改为应答前先排空请求体。

### 14.7 已在真实浏览器中验证

HTTP 200 不等于页面可用——正如 `validate` 通过不等于能回放。以下均在
Chrome 中对真实服务实测：

- 四个面板正确渲染，步骤列表显示 kind / action / `disambiguation.strategy`；
- 点击选中步骤，详情面板显示 locator 与 `unique (verified)`；
- 改 locator 为 `{"role":"button","name":"Store"}` → 标签转为 `unresolved`、
  出现红色告警、编译面板转为 `RECORDING.LOCATOR_UNRESOLVED` 并指名 `click_save`；
- Undo 还原 locator 与可编译状态；
- **原生 HTML5 拖拽重排生效**：`click_save` 拖至第二位，顺序变更、
  出现 `ORDER_DIVERGED` 并列出原始顺序、编译产物的步骤序列随之改变；
- 把 `click_save` 包进条件 `${{ inputs.should_save }}`，编译产物由 9 步收敛为 7 步；
- 非法 JSON 的 locator → 「Edit refused」并给出原因，录制不变。

### 14.8 仍未落地

- §5 表格中的**回放报告视图**未实现，当前第四个面板是「编译结果」；
- §5 提到的**显式单步试跑**端点未实现——路由集中没有任何执行入口，
  这也意味着 UI 当前无法绕过 policy 检查；
- 拖拽仅对顶层步骤开放：跨容器移动 logic 子步骤会改变它属于哪个分支，
  那是另一种操作，未实现。
