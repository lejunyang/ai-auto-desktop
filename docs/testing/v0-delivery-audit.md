...
- Linux 真实 KDE 写动作：发行版 KCalc 22.12.3 在禁用 TCP、使用一次性 Xauthority 的私有 Xvfb，以及私有 KWin、session/AT-SPI bus 和临时 HOME/XDG 中以精确 PID 启动；测试从每次 fresh、未截断快照定位 `1`、`+`、`2`、`=`，分别用 `Action.do_action` exact `Press` 与显式 `pointer_click` 完成 `1+2=3`。pointer 路径由 fresh bounds 推导中心点，经 AT-SPI subtree hit-test 与 X11 PID/focus 复核后用 XTEST 派发，最终从 fresh snapshot 唯一读取同一显示控件的结果 `3`。未使用 OCR、截图或用户配置目录。
