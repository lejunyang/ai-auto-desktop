...
| macOS AX adapter | `crates/aad-macos-ax` | Rust adapter 负责协议、契约、快照/locator/stale 规则；代码签名 Swift helper 保留为 AX/TCC 原生边界 |
...
Rust macOS adapter 保持原 8 个 action contract：`list_apps`、`snapshot`、`find`、`focus`、
`invoke`、`pointer_click`、`set_value`、`type_text`。Swift helper 的 bundle/signature 校验、协议 v2
握手、进度帧与 unknown-effect 边界由 Rust 管理；原生 AX/CGEvent 调用仍留在代码签名 `.app` 中，
从而保持稳定 TCC 身份，仓库无需为该能力保留 Python。
...
## 2.43 界面上的「试一下」，与一条错了很久的窗口判据

### 按钮跑当前编辑中的录制，不要求先存盘

上一节把后端接通了，但只有 CDP 调过——界面上还没有入口。`bridge` 加 `runWorkflow`，
标题栏加 `▶ Try it`。

一个取舍：**跑的是当前编辑中的录制，不先存盘**。这个按钮的用处是「改一步试一步」，
每次都落盘会把一次试错变成一串文件。编译走的是保存时同一条路（`recording.toDescriptor`）。

结果面板按步显示，而不只显示总状态——后端已经给了每步，而「跑了但点错元素」在总状态里
看不出来，那正是在编辑器里回放的意义。失败时把 `effect` 也说出来：值写了没提交，与什么都
没做，重试的安全性不同。

### 点按钮立刻抓到一条错很久的判据

第一次点，界面报 **`the window selector matched 8 windows`**。

追下去发现 `selectorFor` 把 `class_name` 排在第一位，单窗口时就用它「唯一」。而
`isDurableClassName` 问的是**「这个值跨重启还在吗」**——Chrome 的确在。该问的是
**「这个值能把这个窗口从别的窗口里分出来吗」**——不能。**稳定不等于有区分度。**

实测本机 24 个窗口：

| class_name | 窗口数 |
| --- | --- |
| `Chrome_WidgetWin_1` | 8 |
| `WindowsForms10.Window.8.app.0.2bf8098_r6_ad1` | 6 |
| `CASCADIA_HOSTING_WINDOW_CLASS` / `CabinetWClass` / `Windows.UI.Core.CoreWindow` | 各 2 |

**20/24（83%）与别人共用。**

而 Rust 侧 `selector_for` 早就只用 `process_name` + `title`，不唯一就返回 `None`。
**609b483 的教训在移植时修对了，TS 原版没改。** 这次是让 TS 追上 Rust。

**代价量过才改**：原本靠 class_name 才唯一、去掉后就不行的 —— **0 个**。剩下 6 个识别不了的
是同名重复窗口（4 个 `GlassPanelForm` + 2 个「Windows 输入体验」），它们 class_name 也相同，
旧方案同样识别不了，只是会**静默返回一个错的选择器**。所以这次只去掉了静默出错的路径。

**四个测试失败，它们断言的正是我刚否决的行为。** 其中一条的注释给出反证：「24 个窗口里只有
WinForms 那个是易变的」——这与我的数据不冲突，两者问的不是同一件事，而一个选择器要成立
**两件事都得满足**。旧判据只检查了前一半。测试改为编码正确的判据，并保留旧观察说明它为何
不足够。

改后真机：选择器变成 `{"process_name":"msedge.exe","title":"AAD Complex Fixture"}`，26 个
窗口里唯一，界面报 `Ran | 6 of 6 steps`，CDP 核实 `city='FromButton'`、`log=[1] page-save:`。

（中途「匹配 3 个窗口」是探针只传一个 `openWindows` 造成的；真实录制传全部窗口。两次
「按钮 disabled 但 canExport 为 true」都是 Vite 热更新时序，刷新即一致。）

### 截图才看见的第二个问题

六步的面板看着没事，量完才知道会出事：**每行 15px + 54px 开销**。

| 录制步数 | executed | 面板高 | 占窗口 |
| --- | --- | --- | --- |
| 5 | 15 | 279px | 31% |
| 10 | 30 | 504px | 56% |
| 20 | 60 | 954px | **107%** |

而三栏的 `bottom_cut` 已经是 0，没有余量可让——20 步就会把它们整个挤出屏幕。

给步骤列表加 `max-height: 12rem`（12 行 = 4 个逻辑步骤，够看清一次典型回放的全貌）。
另加一条：**把失败的那一步滚进视野**——列表一旦滚动，高亮一个在折叠线以下的行等于没做，
而 20 步录制里那一行通常不在前 12 个。

真机验证 60 步回放：面板 **24%**、列表自己滚、三栏 `[0,0,0]` 一点没被切、页面无溢出。

## 2.44 分区概览与下钻，以及一个前端从未读过的字段

### 先量，结果推翻了我自己的立项理由

我原以为 B 是「GUI 的 `limit=120` 太小」。量完是 **0/23 窗口超过 120**，最大 80 —— 那个理由不成立。

但同一次测量给出了真相，而且更要紧：**`stopped_by=characters`**。给 `limit=2000` 也只返回 59 个
元素，因为**字符预算先撞上**（`OUTLINE_BUDGET=20_000`）。**限制元素数从来不是瓶颈。**

| 窗口 | 看到 | 可达 | 节点 | 占比 |
| --- | --- | --- | --- | --- |
| StoragePlugin.kt - VS Code | 80 | 271 | 671 | **29%** |
| 文件资源管理器 | 52 | 210 | 467 | 24% |
| rust-port-status.md - VS Code | 59 | 167 | 365 | 35% |

**9/23 窗口因字符预算停下**，最坏只看到 **29%**。而 GUI 既没有 `overview` 也没有下钻入口，
所以那 100+ 个元素在界面上**摸不到**。

**下钻是有效的**：前 6 个分区分别取，共拿到 **213 个**，而整体列表只给 80。

### 顺序由数据定

后端加 `overview_window`，`describe_window` 加 `region`；前端在中栏加分区列表。

**取舍一：选窗口时同时要 outline 和 overview**，不等发现截断了再补一次。截断这件事只有 outline
回来才知道，那时再请求会让用户先看到一个不完整的列表、再突然重排。

**取舍二：分区只在需要时领先。** 截断了才把分区推到前面（`leadWithRegions`）；14/23 装得下的窗口
上分区是多余一步，就不挡路。

### 截图才看见的挤压

下钻工作正常后截图，看到**分区列表和元素列表纵向堆叠、各有滚动条**。量出来：分区占 **252px**
（771px 面板的三分之一），元素列表只能看 **7/18 行**。

下钻后分区列表的唯一用途是「换一个分区」，而面包屑已经能回退。所以折叠成一行，加
`switch region` 按钮。折叠后元素列表 **421px → 673px，可见行数 7 → 12**。

### 一个驱动一直在返回、前端从未声明的字段

写测试时类型检查报 `Property 'stopped_by' does not exist on type 'Outline'`。

驱动一直返回它，**前端类型里没有，所以前端无从区分两种截断**：

- `stopped_by=limit` → 提高 limit 有用
- `stopped_by=characters` → 提高 limit 无用，只能下钻（实测 9/23 是这种）

补上类型，并让界面按它分开措辞——告诉用户「更长的列表也没用」和「列表被截断了」不是一回事。

### 一次自造的假缺陷

中途探针报 `region` 有值而 `outline` 为 null，看着像「下钻后列表没填回、用户点一次就卡死」。

追下去是**我的探针写了 `state.failure.value = null`**，而 **setupState 里的 ref 已被解包成裸值**——
之前读到的 `region: "string"` 就是值本身。改成只调函数（`select`/`drillInto`）、只读裸值后，
完整流程一次通过：59（truncated）→ 下钻 18（`truncated: false`）→ 回退 59。

**是探针的问题。** 记入本节，因为 `bridge` / `recording` / `capture` 三者在 setupState 里的形状
各不相同，这个坑会再踩。


### 2.45 拆开最大的那一块分区

**上一节留下的缺口**：`(loose in the window)` 在多数窗口里是最大一块，下钻它仍然
`stopped_by=characters`。分区做完了，最大的一块仍然进不去。

**第一轮测量修正了我给这件事的定性。** 我原先说「进不去」，实测是：15 个有 loose 的窗口里
只有 3 个下钻后仍被截断，而且都拿到了 76~88%（声明 89 拿到 68、82 拿到 66、76 拿到 67）。
严重程度远低于我说的。

**一处矛盾必须先查清**：探针报 `role` 全是 `None`、有名字的 `0/68`，而 overview 明确报
12 button、8 text。**是我读错了字段**——`role` / `name` 在 `locator` 里，不在元素顶层。
字段完整无缺，「拿到 68 个无法辨认的东西」是假的。

**顺着这个读法查预算被什么吃掉，两个猜测被连续否掉**：

- **猜测一：标题回声占了大头。** 实测只占 1~6%（735 / 1240 / 392 字符），不是瓶颈。
- **猜测二：`summary` 是 `locator` 的冗余。** 分布是 `summary` 25% + `locator` 25%，看着是
  同一份信息两种写法。第一版重建函数把 `name="系统"` 算进「剩余」——**那是我的重建漏了
  name**（summary 里 name 可能被截断，字符串替换匹配不上）。改成按 key 比对后结论反转：
  两边各有对方没有的东西，`summary` 独有 `value`（34 次），`locator` 独有 `nth`(48) /
  `within`(100)。`summary` 说「看到了什么」，`locator` 说「怎么再找到它」，删任一个都丢信息。

**无争议的浪费只有两处，但都不解决问题**：`ref` 的 `snapshot:revision` 前缀 37 字符 × 每条、
66/66 完全相同（11%）；`protected` 全机 694/696 是 false（6%）。去掉两者省 18~19%、
多装 12~13 个——3 个窗口里只有 1 个因此装得下。而且 `ref` **不能动**：grep 确认 GUI 显示它、
`act` 用它传参、`driver.rs` 与 `model.rs` 三处断言它，它是「引用你真看过的元素」这个安全模型的载体。

**回到 loose 本身，找它为什么大**：自己走祖先链，**67/68 全部因为「第一个具名祖先是标题回声」
落进 loose**（剩 1 个完全没有具名祖先）。单一成因。

**往上再找一层这条路被数据否掉**：跳过所有标题回声祖先后，**0/68、0/66 能找到别的具名祖先**——
一路到根都是回声或无名。

**按 role 分组可行**：13~15 组，最大 `group` 18~20 个，按字符量算 0 组超预算（最大 7220 = 预算的 36%）。

**但先量准问题规模，才决定值不值得做**：用 `--out` 拿全量对照截断结果，**被截掉的全部有名字**
（22/22、16/17、10/10），含 `edit`、`hyperlink`、`list_item` 等真正可交互的，**不是布局垃圾**；
而且它们全在列表末尾（第 68~89 个），是顺序截断，**最后那批每次都看不到**。是真缺口。

**分区名的命名空间不能混。** 注释明说分区名刻意与 `synthesize` 的 `within` 一致，让「下钻到
Billing address」和「写 `within: {name: "Billing address"}`」说的是同一件事。`role` 不是容器名，
所以 role 细分做成 loose 内部的第二级，写成 `"(loose in the window) / button"`，带分隔符
一眼能看出不是容器名。

**取舍：拆开后不再保留整块 loose。** 留着它就等于留着那条会截断的路，用户点它仍然只看到
三分之二——**一条走不通的路不如不给**。

#### 修好一个上限，却撞上另一个

第一版实测：每个子分区声明数=拿到数、0 组超预算，但覆盖率只有 68% / 80% / 79%。
我当时归因为 `OVERVIEW_REGIONS = 12` 把 13~15 组截到 12。

**量完发现这个归因也是错的**：折叠掉的是 82~118 组，不是几组，而且折叠本来就存在
（`docs(agents)` 折叠 20 组，它根本没拆 loose）。

**一个自造的假数据**：我写探针用 `locator.within` 反推分区，得出「380/402 未列出」。
但 `within` 只在需要消歧时才合成，**352 个元素的 within 是 null**，它不是分区归属。
**是探针的问题。**

改成直接问驱动，账目立刻对上：**概览能到达 51~55%，差额精确等于它自称折叠的数量**
（180 vs 179、129 vs 128）。

**真因是拆分制造了新的竞争**：15 个子分区与其他容器抢同 12 个席位，结果两边都输——
5 个子分区赢了席位，**4 个具名容器被挤掉**，而 loose 里仍有 28 个元素（31%）摸不到。

**修法：子分区不与具名容器竞争席位。** 理由是性质不同——具名容器是「窗口里的一块地方」，
子分区是「剩下的东西按种类归堆」，后者是兜底，不该挤掉前者。改后两边都拿回来了：
12 个具名容器全部保留，子分区 5 → 10 组，覆盖 62 → 82/90（91%）。

**曲线否掉了「加大 OVERVIEW_REGIONS」这条路**：12 → 20 组只从 79% 涨到 82%，
而概览字符翻倍（1272 → 2120）。收益递减，12 这个值是对的。

#### 覆盖率判据会把「少但重要」排到长尾

按元素数覆盖 90% 之后，仍有 **5 个 `edit`、1 个 `check_box`、2 个 `list_item`** 被折叠。
输入框是最该能操作的东西，而它按数量排在长尾。

**代价先量再改**：把接受输入的组一律列出，平均只多 1.2 组（约 127 字符），换回 9 个可交互元素。
判据用节点自己声明的动作（`set_value` / `type_text`），**不用 role 白名单**——白名单会漏掉
平台特有的控件类型，而 actions 是驱动实测出来的。改后 5 个 `edit` 全部拿回，缺口 9 → 3。

**剩下 3 个到此为止是对的**：查它们支持什么动作，`list_item` 与 `check_box` 只有
`focus` / `invoke` / `pointer_click`——和 `pane`、`header`、`window` 完全一样。而 `invoke`
几乎人人都有（§2.44 已量过 131 个里 124 个），按它筛等于不筛。**没有信号能把它们与布局容器分开。**

#### 只有截图才看见的第三个问题

分区列表 `max-height: 18rem` 是分区≤12 时定的死值。拆分把分区推到 24，死值就成了瓶颈：
**24 个只露出 6 个，而面板下方 456px 完全空着**（够放 11 行）。
