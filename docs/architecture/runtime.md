...
继承给该 worker；Windows Host 创建随机命名的单实例 duplex named pipe，以 protected DACL
只允许当前用户与 LocalSystem，并在连接后核验 client PID，worker 也核验 server PID。图片原始
字节使用 `AADF` v1 有界帧传输，不做 base64，也不与 stdout
...
ArtifactStore 仍只接受可信内置 producer 的图片：POSIX 使用 fd-relative 私有目录，Windows
使用不可变且受总量/单项配额限制的 run-scoped 内存后端。Windows 首版不支持路径导入、磁盘
spill 或重启恢复；第三方不可信图片解码仍需迁移到受限 worker。ArtifactRef 是本次 live
execution scope 的能力，不可作为跨进程重启的 durable
